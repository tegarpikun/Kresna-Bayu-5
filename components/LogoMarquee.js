...
export default function LogoMarquee() {
  ...
  return (
    <div className="border-t border-welcome-border py-10">
      <p className="mb-8 text-center font-warm text-[10px] font-semibold uppercase tracking-[0.3em] text-welcome-primary/70">
        ...
        <div className="marquee-track flex w-max items-center gap-16">
          {track.map((logo, i) =>
            logo.src ? (
              ...
              <span
                key={`${logo.name}-${i}`}
                className="whitespace-nowrap font-display text-xl italic tracking-wide text-welcome-textSoft/60 transition-colors hover:text-welcome-primary"
              >
                {logo.name}
              </span>
            )
